 ...
 */
 ...
 public class BinaryTree {
     ...
     private int findMinHelper(Node node) {
         ...
         int min = findMinHelper(node.left); // min of left child
         int right = findMinHelper(node.right); // min of right child
     ...
     private int nodesGTHelper(Node node, int val) {
        ...
        if (this.root == null) { // if there's no tree (this.root doesn't exist), return -1
            return -1;
        }
         ...
         *  if node.data > val, the method will also add 1 when it returns left + right to
         *  increment the total count of nodes greater than val.
         */
        if (node.data > val){
            return left + right + 1;
        }
