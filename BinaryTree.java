...
public class BinaryTree {
    ...
    private void replaceValueHelper(Node node, int oldVal, int newVal) {
        ...
        if (node == null) { // base case for null
            return;
        }

        /*
         * traverse as far left as possible, replacing node.data with newVal when
         * node.data = oldVal, and then traversing to the right.
         */
    ...
    private int findMinHelper(Node node) {
        ...
        int min = findMinHelper(node.left); // min of LEFT sub-tree of current node (called min for convenience)
        int right = findMinHelper(node.right); // min of RIGHT sub-tree of current node

        /*
         * return the lowest value between the minimum of the left + right child
    ...
    private int nodesGTHelper(Node node, int val) {
        ...
        if (this.root == null) { // if there's no tree(root node), return -1
            return -1;
        }
        if (node == null) { // base/leaf case
            return 0;
        }
        int left = nodesGTHelper(node.left, val); // # of nodes greater than val in left sub-tree of current node
        int right = nodesGTHelper(node.right, val); // # of nodes greater than val in right sub-tree of current node
        /*
         * the method returns the # of nodes greater than val in the
         * current node's left and right subtrees (which will start at 0 at leaf nodes).
         * if node.data > val, the method will also add 1 when it returns left + right
         * to increment the total count of nodes greater than val.
         */
        if (node.data > val) {
            return left + right + 1;
        }
    ...
    private int[] averageHelper(Node n) {
        ...
        int[] left = averageHelper(n.left); // sum + count of left sub-tree
        int[] right = averageHelper(n.right); // sum + count of right sub-tree
    
        /*
         * return current total sum + current node's data, and node-count + 1
